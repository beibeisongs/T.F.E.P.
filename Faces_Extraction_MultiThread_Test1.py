...
def extract_Face_InsideJPG(path_jpg, pic_id, detector, document_path,):
    ...
    face_number = 0
    print("extract_Face_InsideJPG Step 1 Ok!")

    try:
        img = io.imread(path_jpg)
        print("extract_Face_InsideJPG Step 2 Ok!")
        ...
        # 人脸检测
        dets = detector(img, 1)
        print("extract_Face_InsideJPG Step 3 Ok!")
        ...
        print("extract_Face_InsideJPG Step 4 Ok!")

        # 得到人脸个数的变量
        num_of_faces = len(dets)
        print("extract_Face_InsideJPG Step 5 Ok!")
...
def MultiThread_ReadLines(path, get_user_id, pic_id, document_path):

        # 构造要读的jpg文件的完整路径：
        path_jpg = compose_ReadJPG_Path(path, get_user_id, pic_id)  # 这个path就是要读的JPG的完整路径
        print("MultiThread_ReadLines Step 1 Ok!")

        # 现在开始检测并记录该图片中的人脸
        extract_Face_InsideJPG(path_jpg, pic_id, detector, document_path)
        print("MultiThread_ReadLines Step 2 Ok!")

def start_SubThread(path, get_user_id, pic_id, document_path):

    class myThread(threading.Thread):

        def __init__(self, path, get_user_id, pic_id, document_path):
    ...
    Thread1 = myThread(path, get_user_id, pic_id, document_path)
...
def read_JsonFiles(path, get_user_id, dirpath):
    ...
    f1 = open(path, encoding='utf-8')
    print("read_JsonFiles Step 1 Ok!")
    ...
    print("read_JsonFiles Step 2 Ok!")

    line_i = 0

    sub_i = -1   # 这个变量，所存放的整形值，加到5就重新变成0

    Threads = []    # 这是一个list，用于存放已经开启的线程
    ...
    for line in f1.readlines():
        ...
        print("read_JsonFiles Step 3 Ok!")
        ...
        if pic_id != "F":   # 即没有捕捉到异常
            ...
            if (line_i < 6):
                ...
                Threads.append(start_SubThread(path, get_user_id, pic_id, document_path))
    ...
    while (mark == True):
        ...
        if m2 == True:
            ...
            print("等一秒！")
            time.sleep(1)   # 即有subThread在运行，先睡一秒
