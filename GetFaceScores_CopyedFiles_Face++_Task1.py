...
for dirpath, dirnames, filenames in os.walk(path):
    for filepath in filenames:  # <Sample>: filepath = '1982819117.json'
        ...
        get_user_id = get_user_id.split("\\")
        get_user_id = get_user_id[0]

        filepath = filepath.split(".")
        fileformat = filepath[1]
        jpgname = filepath[0]
        ...
        if fileformat == "jpg":
            ...
            if (AccountFileNumber >= start_pt) and (AccountFileNumber <= end_pt):
                ...
                jpgFilename = jpgname + "." + fileformat

                belongerFace_JPG_Wholepath = path + "\\" + get_user_id + "\\" + get_user_id + "\\" + jpgFilename
                ...
                json_FaceScore_TheWholePath = dirpath + "\\" + "Face_Scores.json"

                f1 = open_Json_File_To_Write(json_FaceScore_TheWholePath)

                write_Score(req_dict, f1, jpgname)
