...
def makeUpDetection(belonger_facepath, dirpath, jpg_name):
    ...
    f1 = open(dirpath + "\\" + jpg_name + "_makeup_result.json", mode='w')
    ...
    for dirpath, dirnames, filenames in os.walk(path):
            ...
            if len(path_Divided) == 5:
                ...
                if AccountFileNumber >= start_pt:
                    ...
                    re_json_path = dirpath + "\\" + get_user_id + "_results.json"
                    if os.path.exists(re_json_path):

                        belong_jpg_name = readResultJson(re_json_path)
                        if len(belong_jpg_name) == 1:
                            jpg_dir_list = os.listdir(dirpath + "\\" + belong_jpg_name[0])
                            for jpg_name in jpg_dir_list:
                                if jpg_name[0] != 'F':
                                    jpg_name_p1 = jpg_name.split('.')
                                    jpg_name_p = jpg_name_p1[0]

                                    belong_face_path = dirpath + "\\" + belong_jpg_name[0] + "\\" + jpg_name
                                    makeUpDetection(belong_face_path, dirpath, jpg_name_p)
                        else:
                            print("Info: No face ! ")
