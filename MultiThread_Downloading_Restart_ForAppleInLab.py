...
def Download_Pics(the_path_to_download_jpg, get_composed_url, shushu):
    class SubThread():
        ...
        def sub_Thread_Download(the_path_to_download_jpg, get_composed_url, shushu, sub_thread1,pic_restart_point):
    ...
    t = threading.Thread(target=SubThread.sub_Thread_Download, name=None,
                         args=(the_path_to_download_jpg, get_composed_url, sub_thread1.shushu, sub_thread1,pic_restart_point))
    t.start()
    t.join()
...
pic_num_least = input()
print("请输入重新下载的起点：（即下载到的图片张数）")
pic_restart_point = input()
"""
input_province = "湖北省"
input_city = "武汉市"
input_year = "2014"
